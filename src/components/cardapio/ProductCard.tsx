...
const ProductCard = ({ product }: ProductCardProps) => {
  ...
  return (
    <div className="group relative overflow-hidden rounded-lg border border-stroke bg-white shadow-sm transition-all hover:shadow-md dark:border-stroke-dark dark:bg-gray-dark">
      ...
      {!product.isAvailable && (
        <div className="absolute right-3 top-3 z-10 rounded-full bg-red px-3 py-1 text-base font-medium text-white">
          Indisponível
        </div>
      )}
      ...
      <Link href={`/cardapio/produto/${product.id}`} className="block">
        <div className="p-4 pb-2">
          ...
          {/* Descrição */}
          <p className="mb-3 line-clamp-2 text-base text-body dark:text-gray-5">
            {product.description}
          </p>
          ...
          {product.ingredients.length > 0 && (
            <div className="mb-3">
              ...
              <p className="line-clamp-1 text-sm text-body dark:text-gray-5">
        ...
        {product.isAvailable && (
          <div className="flex items-center justify-between gap-3">
            ...
            <button
              ...
              className="flex-1 rounded-xl border-2 border-gray-300 bg-white px-4 py-2 text-base font-semibold text-gray-900 transition-all hover:bg-gray-100 hover:shadow-md dark:border-gray-600 dark:bg-gray-800 dark:text-white dark:hover:bg-gray-700"
            >
              Adicionar
            </button>
          </div>
